...
public class ModCreativeModeTabs {
    public static final DeferredRegister<CreativeModeTab> CREATIVE_MODE_TABS =
            DeferredRegister.create(Registries.CREATIVE_MODE_TAB, Todolist.MODID);

    // 注册本模组的创造模式标签。
    // "TODO_LIST_TAB" 为该标签的注册名，会作为内部 ID 使用。
    public static final RegistryObject<CreativeModeTab> TODO_LIST_TAB =
            CREATIVE_MODE_TABS.register("todolist",
                    () -> CreativeModeTab.builder()
                            // 设置创造标签在界面中显示的图标
                            .icon(() -> new ItemStack(Items.STONE))
                            // 设置标签的显示名称
                            .title(Component.translatable(Todolist.MODID + ".tab.todolist"))
                            // 定义该标签中显示的物品内容
